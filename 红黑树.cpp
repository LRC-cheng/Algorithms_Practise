...
void RBDeleteFixUp(RBTree &T, RBTree x){
	...
	while (x != T&&x->data.Color == BLACK){		//顶替结点x不为根结点，且x(y)为黑色时才要调整位置
		if (x == x->parent->left){
			w = x->parent->right;
			if (w->data.Color == RED){
				w->data.Color = BLACK;				//case 1  兄弟结点为红色，把兄弟结点变为黑色，使x处补充一个黑色
				x->parent->data.Color = RED;
				LeftRotate(T, x->parent);
				w = x->parent->right;
			}
			if (w->left->data.Color == BLACK&&w->right->data.Color == BLACK){
				w->data.Color = RED;				//case 2  兄弟黑色，其孩子也是黑色，把兄弟变红色（减少旁边一个黑色结点数）
				x = x->parent;
				if (x == T){
					break;
				}
				...
				w->left->data.Color = BLACK;		//case 3 兄弟左孩为红，先调整为情况四，再补充结点
				w->data.Color = RED;
				RightRotate(T, w);
				w = x->parent->right;
			}
			w->data.Color = x->parent->data.Color;	//case 4 ，兄弟为黑，且右孩子为红，利用右树为左树补充一个黑结点
...
void RBInsertFixUp(RBTree &T, RBTree z){
	...
	while (z->parent->data.Color == RED){
		if (z->parent == z->parent->parent->left){
			...
			else if (z == z->parent->right){		//情况二
				z = z->parent;
				LeftRotate(T, z);
			}
			z->parent->data.Color = BLACK;			//情况三
			z->parent->parent->data.Color = RED;
